...
parser.add_argument("--arc_budget", default=2, type=int, help="arc budget count")
...
if dataset == "synthetic_uniform" or dataset == "power_law_53":
    ...
    #a = np.load("network/{}/a_{}.npy".format(dataset, args.dataset_id))
    ...
    print("Perturbed Nodes: ", perturb_index)
    y[perturb_index] = 0
elif dataset in ["USairport_2010","USairport_2010_66","USairport_2010_34","USairport_2010_16"]:
    a = np.load("{}network/{}/a.npy".format(prefix, args.dataset))
    n = a.shape[0]
    print("node count:", n)
    budget = args.budget
    arc_budget = args.arc_budget
...
sample_dir = "{}_{}_{}_{}_{}_{}".format(n,intact_num,budget,arc_budget, args.coef_c, args.coef_u)
...
def get_neq_constrs(n, delta_y, delta_a):
    """
    input: 
        n:  scalar (# of nodes)
    ...
    for i in range(n):
        for j in range(i+1,n):
            neq_constr_coefs[constr_id, n*i+j] = 1
            neq_constr_rhs.append(u[i,j] * (y_init[i] + delta_y[i]))
            constr_id += 1

            neq_constr_coefs[constr_id, n*i+j] = 1
            neq_constr_rhs.append(u[i,j] * (a[i][j] + delta_a[i][j]))
            constr_id += 1

    for i in range(n):
        for j in range(i+1,n):
            neq_constr_coefs[constr_id, n*i+j] = 1
            neq_constr_rhs.append(u[i,j] * (y_init[j] + delta_y[j]))
            constr_id += 1
...
def get_dual_problem(n, delta_y=None, delta_a=None):
    """
    max -c1 z1 + -c2 z2
    s.t. [A11, A12] z == -b1
            [A21, A22] z >= -b2
    ...
    _, neq_constr_coefs, neq_constr_rhs = get_neq_constrs(n, delta_y, delta_a)
...
def phi_y(y, a_list):
    ...
    n = len(y)
    n_variables_positive = 2*n
    n_neq_constrs , _ , neq_constr_rhs = get_neq_constrs(n, y, a_list)
...
def topk_y(z_iter, y_init, a_init):
    m=Model('mip_0')
    ...
    a_list = [[-1 for _ in range(a_init.shape[1])] for _ in range(a_init.shape[0])]
    for i in range(a_init.shape[0]):
        for j in range(i+1, a_init.shape[1]):
            a_list[i][j] = m.addVar(name="a_{}_{}".format(i,j), vtype=GRB.BINARY)
    ...
    m.addConstr((quicksum(a_list[i][j] for i in range(a_init.shape[0]) for j in range(i+1, a_init.shape[1])) <= arc_budget), name="ca_2")
    m.addConstrs((a_init[i][j] + a_list[i][j] <= 1 for i in range(a_init.shape[0]) for j in range(i+1, a_init.shape[1])), name="ca_1")
    dual_obj_coefs = phi_y(y, a_list)
    ...
    delta_a_opt = np.zeros((a_init.shape[0], a_init.shape[0]))
    for v in m.getVars():
        if (v.varName[0] == 'y'):
            idx = int(v.varName.split('_')[-1])
            y_opt[idx] = v.x
        elif (v.varName[0] == 'a'):
            idx1, idx2 = int(v.varName.split('_')[-2]), int(v.varName.split('_')[-1])
            delta_a_opt[idx1, idx2] = v.x
            #print(idx1, idx2, v.x, a_init[idx1,idx2], a[idx1,idx2])
        else:
            assert False
    subgrad = -phi_y(y_opt, delta_a_opt)
    return m_i0, y_opt, delta_a_opt, subgrad
...
delta_a_init = np.zeros((n,n))
for _ in range(arc_budget):
    delta_a_idx_init_1 = np.random.choice(n-1, size=1)
    delta_a_idx_init_2 = np.random.choice(np.arange(delta_a_idx_init_1+1, n), size=1)
    if a[delta_a_idx_init_1, delta_a_idx_init_2] != 1:
        delta_a_init[delta_a_idx_init_1, delta_a_idx_init_2] = 1
...
def presolve_dual():
    ...
    dual_n_variables, dual_n_obj_coefs1, _, dual_obj_coefs,\
            dual_eq_constr_coefs, dual_neq_constr_coefs,\
            dual_eq_constr_rhs, dual_neq_constr_rhs,\
            dual_n_eq_constrs, dual_n_neq_constrs, _ = get_dual_problem(n, delta_y_init, delta_a_init)
...
z_init = np.load("problem_set/{}/{}/z_init.npy".format(dataset, sample_dir))
...
def presolve_primal():
    ...
    delta_y = {}
    for i in range(n):
        #delta_y[i] = m.addVar(lb = 0, ub = 1, name="y_{}".format(i))
        delta_y[i] = m.addVar(name="y_{}".format(i), vtype=GRB.BINARY)


    delta_a = [[-1 for _ in range(a_init.shape[1])] for _ in range(a_init.shape[0])]
    for i in range(a_init.shape[0]):
        for j in range(i+1, a_init.shape[1]):
            delta_a[i][j] = m.addVar(name="a_{}_{}".format(i,j), vtype=GRB.BINARY)
    m.addConstr((quicksum(delta_a[i][j] for i in range(a_init.shape[0]) for j in range(i+1, a_init.shape[1])) <= arc_budget), name="ca_2")
    m.addConstrs((a_init[i][j] + delta_a[i][j] <= 1 for i in range(a_init.shape[0]) for j in range(i+1, a_init.shape[1])), name="ca_1")
    
    m.addConstrs((y[i] + delta_y[i] <= 1 for i in range(n)), name ='cy_1')
    m.addConstr((quicksum([delta_y[i] for i in range(n)]) <= budget), name='cy_2')

    n_neq_constrs, neq_constr_coefs, neq_constr_rhs = get_neq_constrs(n, delta_y, delta_a)
    ...
    for v in m.getVars():
        if (v.varName[0] == 'y'):
            ...
            idx = int(v.varName.split('_')[-1])
            var_arr[idx] = v.x
    return var_arr, y_arr, delta_a_opt, m.objVal

gurobi_start_time = time.time()
var_gt, y_gt, delta_a_gt, obj_gt = presolve_primal()
print("optimal objective:", obj_gt)
print("optimal delta a:", np.where(delta_a_gt != 0))
print("optimal revocer:", np.where(y_gt!=0))
print("gurobi time: {}".format(time.time()-gurobi_start_time))

np.save("problem_set/{}/{}/var_gt.npy".format(dataset, sample_dir), var_gt)
np.save("problem_set/{}/{}/y_gt.npy".format(dataset, sample_dir), y_gt)
np.save("problem_set/{}/{}/delta_a_gt.npy".format(dataset, sample_dir), delta_a_gt)
np.save("problem_set/{}/{}/obj_gt.npy".format(dataset, sample_dir), np.array(obj_gt))
...
def test0(rho, beta,alpha,iter_num):
    ...
    s_t_sum  = rho * np.sum(s_list[0] - x_list[0], axis=0)
    gamma_t_sum = np.sum(gamma_list[0], axis=0)
    ...
    for it in range(1, iter_num):
        ...
        s_t =s_list[-1].copy()
        if it < 10000:
            coef_sum = 1/np.sqrt(10000)
        else:
            coef_sum = 1/np.sqrt(it)
        for i in range(1):
            topk_start_time = time.time()
            objVal, y_opt, delta_a_opt, subgrad = topk_y(x_t, y_init, a_init)
        ...
        print("y_opt")
        print(np.where(y_opt))
        print("delta a opt")
        print(np.where(delta_a_opt != 0))      
    ...
    np.save("results/{}/{}/".format(dataset,"{}_".format(file_name) + sample_dir) +\
                 "{}_rho{}_beta_{}_alpha_{}_iter_{}_x_2.npy".format(file_name, rho, beta, alpha,iter_num), x_t)
    np.save("results/{}/{}/".format(dataset,"{}_".format(file_name) + sample_dir) +\
                 "{}_rho{}_beta_{}_alpha_{}_iter_{}_s_2.npy".format(file_name, rho, beta, alpha,iter_num), np.array(s_t))
    np.save("results/{}/{}/".format(dataset,"{}_".format(file_name) + sample_dir) +\
                 "{}_rho{}_beta_{}_alpha_{}_iter_{}_gamma_2.npy".format(file_name, rho, beta, alpha,iter_num), np.array(gamma_t))
    ...
    plt.savefig("results/{}/{}/".format(dataset,"{}_".format(file_name) + sample_dir) +\
                 "{}_rho{}_beta_{}_alpha_{}_iter_{}_1_2.png".format(file_name, rho, beta, alpha,iter_num))
    
    plt.figure()
    plt.plot(constr_list[10:])
    plt.title("min:{:.6f} last:{:.6f}".format(np.min(constr_list), constr_list[-1]))
    plt.savefig("results/{}/{}/".format(dataset,"{}_".format(file_name) + sample_dir) +\
                 "{}_rho{}_beta_{}_alpha_{}_iter_{}_2_2.png".format(file_name, rho, beta, alpha, iter_num))
